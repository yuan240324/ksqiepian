...
拿到满意的原片段后，用 `styler/style_clip.py` 包装成可直接发布的快手竖屏成片。
...
横竖屏素材通吃：横屏游戏直播自动转 9:16（画面居中 + 上下模糊延展），
花字 / 字幕条落在延展带上。本轮成品（1080x1920@30）：

| 成品 | 风格 | 素材 | 片长 |
| --- | --- | --- | --- |
| 快手_综艺花字风_笑到失控 | 综艺花字 | 9/13 直播笑场 | 35s |
| 快手_解说盘点风_团战名场面 | 解说盘点 | 9/16 团战推基地 | 35s |
| 快手_土味老铁风_社死瞬间 | 土味老铁 | 9/16 社死弧线（大笑→震惊→捂脸） | 30s |

选段方法（音频能量聚类 + 密抽帧核对）、三风格视觉规范、直播源时间戳抖动导致的
mux EINVAL 修复（pts 量化到 1/30 网格）等见 [`docs/08-快手三风格切片.md`](docs/08-快手三风格切片.md)。
